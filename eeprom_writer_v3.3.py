...
COL_CFG=[
    ...
    ("Description",320,140, 1, True),   # Data: stretch 열
]
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.filename=""
        self.conn=None
        self._device_index=-1     # 선택 장치 인덱스 (-1=미선택)
    ...
    def _build_ui(self):
        ...
        self._pages={}
        for name in["connect","viewer","log","about"]:
    ...
    # CMIS 5.0 기준: Password는 I2C로 4바이트 직접 쓰기
    ...
    def _open_file(self):
        ...
        self.orig_data={k:[0]*128 for k in PAGE_KEYS}  # 파일 로드 시 orig=0
    ...
    def _decoded_p02(self,v,i):
        ...
        # µW → dBm 변환 (CMIS: U16 × 0.1µW)
        def uw_to_dbm(raw_u16):
            uw = raw_u16 * 0.1          # 0.1µW 단위 → µW
            if uw <= 0: return "No signal"
            mw = uw * 1e-3              # µW → mW
            return f"{10*math.log10(mw):+.2f} dBm"
        # Tx Power: U16 × 0.1µW → dBm (48~55)
    ...
    def _desc_p02(self,i):
        ...
        table={
            ...
            48:(_txpw[0],"U16×0.1µW → dBm"),50:(_txpw[1],"U16×0.1µW → dBm"),
            52:(_txpw[2],"U16×0.1µW → dBm"),54:(_txpw[3],"U16×0.1µW → dBm"),
            56:(_txbi[0],"U16×2µA"),58:(_txbi[1],"U16×2µA"),
            60:(_txbi[2],"U16×2µA"),62:(_txbi[3],"U16×2µA"),
            64:(_rxpw[0],"U16×0.1µW → dBm"),66:(_rxpw[1],"U16×0.1µW → dBm"),
            68:(_rxpw[2],"U16×0.1µW → dBm"),70:(_rxpw[3],"U16×0.1µW → dBm"),
    ...
    def _update_dirty_label(self):
        n=sum(1 for pk in PAGE_KEYS for i in range(128)
              or self.data[pk][i]!=self.orig_data[pk][i])
    ...
    def _calc_checksum(self, pk):
        ...
        v=self.data[pk]
        if pk=="p00":
            # Byte 128~221 (index 0~93) 합산 → index 94
            self.data[pk][94]=sum(v[0:94])&0xFF
        elif pk in("p01","p02","p10","p11"):
            # Byte 130~254 (index 2~126) 합산 → index 127
            # (P01h: FW revision bytes 0~1 제외 / 나머지: 전체)
            start=2 if pk=="p01" else 0
            self.data[pk][127]=sum(v[start:127])&0xFF

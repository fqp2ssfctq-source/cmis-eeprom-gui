...
CHANGELOG = [
    ("3.5.8","2026-04-28","patch",  "CP2112 sclLowTimeout=False — NVM 클럭 스트레칭 허용, S1=0x00 false alarm 완전 해결"),
    ("3.5.8","2026-04-28","patch",  "write_burst S1=0x00 임시방편 제거 / set_page 중복 호출 제거"),
    ("3.5.7","2026-04-28","major",  "쓰기: burst write 도입, verify 페이지 단위 read로 개선"),
...
class CP2112I2C:
    ...
    def write_burst(self, i2c_addr_8bit, start_reg, data_list, nvm_delay=0.020):
        """연속 주소 버스트 쓰기 (1 I2C 트랜잭션 단위)
        CP2112 WriteRequest 최대 = 61바이트 (register 1 + data 60)
        → data_list가 60바이트 초과 시 60바이트 청크로 자동 분할.
        청크 간 nvm_delay(기본 20ms) 대기.
        sclLowTimeout=False 설정으로 모듈의 클럭 스트레칭을 CP2112가 자연스럽게 처리.
        """
        self._check_open()
        ...
        while pos < len(data_list):
            chunk = data_list[pos:pos+MAX_CHUNK]
            reg   = (start_reg + pos) & 0xFF
            n     = len(chunk)
            buf   = (ctypes.c_uint8 * (n+1))(reg, *[v & 0xFF for v in chunk])
            st    = self._dll.HidSmbus_WriteRequest(
                self._handle,
                ...
                ctypes.c_uint8(n+1))
            if st != self.HID_SMBUS_SUCCESS:
                raise IOError(f"HidSmbus_WriteRequest(burst) 실패 (0x{st:02X})")
            self._wait_complete()
            pos += n
            if pos < len(data_list):
                time.sleep(nvm_delay)
        return True
...
class SimulatedI2C:
    ...
    def write_burst(self,*a): time.sleep(0.001); return True
        ...
        def task():
            ...
            try:
                ...
                for bi,(pk,idx,start_addr,vals) in enumerate(bursts):
                    ...
                    done_bytes+=n
                    # write_burst 내부에서 청크 간 NVM tWR 대기 처리
                    # 버스트 그룹 간에는 추가 대기
                    time.sleep(NVM_DELAY)

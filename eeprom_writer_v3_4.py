...
CHANGELOG = [
    ("3.4.4","2026-04-23","patch",  "CMIS 5.0 → 5.2 표기 수정 / 스펙 버전 상수화 / _decoded_a0 s16/u16 헬퍼 적용"),
    ("3.4.4","2026-04-23","patch",  "_open_file orig_data 파일값 동기화 버그 수정 / _desc_p02 상수화"),
    ("3.4.4","2026-04-23","patch",  "CmisRevision Decoded 버전명 맵 추가 (v5.2 등)"),
...
class App(tk.Tk):
    ...
    def _start_write(self):
        ...
        # 확인 팝업 요약
        mode_str="전체 쓰기"
        summary_lines=[f"[{mode_str}] EEPROM에 씁니다.\n"]
        cur_pk=None; shown=0
        for pk,idx,addr,val in targets:
            if pk!=cur_pk:
                summary_lines.append(f"  [{pk.upper()}]")
                cur_pk=pk
            orig=self.orig_data[pk][idx]
            orig_str=f"{orig:02X}h" if orig is not None else "??"
            summary_lines.append(f"    {addr:02X}h ← {val:02X}h  (원본: {orig_str})")
            shown+=1
            if shown>=20 and len(targets)>20:
                summary_lines.append(f"    ... 외 {len(targets)-shown}바이트")
                break
        ...
        def task():
            ...
            try:
                ...
                self._log(f"━━━ EEPROM 쓰기 시작 ({mode_str}, {total}바이트) ━━━")
                self.after(0,lambda:self.conn_status.config(
                ...
                self.after(0,lambda:(
                    ...
                    self.progress_label.config(text=f"✓ {done}바이트 쓰기 완료"),
                    self._update_dirty_label(),
                    self._show_page(self.current_page),
                    messagebox.showinfo("완료",f"{done}바이트 EEPROM 쓰기 완료!")))
            except Exception as ex:

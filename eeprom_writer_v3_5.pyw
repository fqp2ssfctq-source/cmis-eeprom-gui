...
CHANGELOG = [
    ("3.5.5","2026-04-28","minor",  "쓰기 후 read-back 검증 자동 실행 — 불일치 바이트 로그+팝업"),
    ("3.5.4","2026-04-28","patch",  "쓰기: dirty 바이트만 전송 (SFF-8636 방식) / 팝업 헤더 드래그"),
...
# P00h (Page 00h) — NVM 벤더 정보, Reserved 바이트 제외
# idx 93  (DDh): Reserved
# idx 119~126 (E7~EEh): Reserved
# idx 127 (FFh): Reserved
# → 위 바이트에 쓰기 시도 시 모듈이 NACK(S1=0x00) 반환
_RW_P00 = (set(range(128))
           - {93}               # Reserved[DDh]
           - set(range(119,127)) # Reserved[E7~EEh]
           - {127})              # Reserved[FFh]
...
_CMP_DEFAULT_EXC = [
    ("a0",   3,  3,  "GlobalStatus (실시간 모듈 상태)",              True),
...
class App(tk.Tk):
    ...
    def _cmp_read_dut(self):
        ...
        def task():
            ...
            try:
                ...
                # Upper Pages
                for pk,pn in zip(["p00","p01","p02","p10","p11"],
                                  [0,1,2,0x10,0x11]):
                    conn.set_page(pn)
    ...
    def _cmp_run(self):
        ...
        바이트 단위 전체 비교
        - Identifier 중복 표시 방지:
          A0h[00h]과 P00h[80h]은 동일 물리 데이터(SFF8024Identifier)이므로
          A0h쪽 한 곳에서만 FAIL/PASS 판정하고, P00h[00]은 건너뜀
        - 예외 바이트: SKIP (값이 다를 때만 Treeview 표시)
        - 예외 아님: FAIL → Treeview, PASS → 카운트만 (표시 안 함)
        """
        ...
        for pk, page_lbl in zip(PAGE_KEYS, PAGE_LABELS):
            ...
            for i in range(128):
                # Identifier 중복 방지: P00h idx=0 은 A0h[0]와 동일 → 스킵
                if pk == "p00" and i == 0:
                    continue
    ...
    def _start_write(self):
        ...
        # ── 체크섬 강제 포함 (P00h/P01h/P02h 선택 시) ───────
        # 변경이 있는 페이지의 체크섬은 항상 함께 써야 NVM이 유효함
        cs_map={"p00":94,"p01":127,"p02":127}
        ...
        for pk,cs_idx in cs_map.items():
            if pk in sel_pks:
                ...
                if has_change and (pk,cs_idx) not in existing:
                    base=0x80
                    targets.append((pk,cs_idx,cs_idx+base,self.data[pk][cs_idx]))
        ...
        def task():
            ...
            try:
                ...
                self._log(f"━━━ EEPROM 쓰기 시작 (dirty {total}바이트) ━━━")
                self.after(0,lambda:self.conn_status.config(
                    text="● 쓰기 중...",fg=self.theme["amb"]))
                cur_pk=None
                pw_sent=False
                nvm_pks={"p00","p02","p10","p11"}
                for pk,idx,addr,val in targets:
                    # NVM 페이지 첫 접근 시 패스워드 자동 재전송
                    if pk in nvm_pks and not pw_sent:
                        try:
                            addrs=self._parse_pw_bytes(self.pw_addr_vars)
                            vals_pw=self._parse_pw_bytes(self.pw_val_vars)
                            for pa,pv in zip(addrs,vals_pw):
                                conn.write_byte(i2c,pa,pv)
                            pw_sent=True
                            self._log("  ↳ NVM 쓰기 전 Password 자동 재전송 완료")
                        except Exception as pe:
                            self._log(f"  ↳ ⚠ Password 재전송 실패: {pe}")
                    if pk!="a0" and pk!=cur_pk:
                        conn.set_page(page_map[pk]); cur_pk=pk
                    conn.write_byte(i2c,addr,val); done+=1
                    pct=done/total*100
                    msg=f"{pk.upper()} [{addr:02X}h] ← {h2(val)} ({done}/{total})"
                    self._log(msg)
                    self.after(0,lambda p=pct,m=msg:(
                        self.progress_var.set(p),
                        self.progress_label.config(text=m)))
                    time.sleep(0.005)
                conn.set_page(0)   # 완료 후 Page 0으로 복귀
                self.orig_data=copy.deepcopy(self.data)
                self._log(f"━━━ ✓ 쓰기 완료 ({done}바이트) ━━━")
                ...
                for pk,idx,addr,wrote_val in targets:
                    if pk!="a0" and pk!=cur_vk:
                        conn.set_page(page_map[pk]); cur_vk=pk
                    read_addr=addr if pk=="a0" else addr
                    rb=conn.read_page(i2c, read_addr, 1)
                    read_val=rb[0] if rb else None
                ...
                if mismatches:
                    ...
                    self._log(f"━━━ ✓ 검증 통과 ({done}바이트 모두 일치) ━━━")
                    self.after(0,lambda:(
                        self.progress_var.set(0),
                        self.progress_label.config(text=f"✓ {done}바이트 쓰기+검증 완료"),
                        self._update_dirty_label(),
                        self._show_page(self.current_page),

...
CHANGELOG = [
    ("3.5.9","2026-04-28","patch",  "쓰기: write_burst 내부를 byte-by-byte로 전환 (모듈 2바이트 write만 지원)"),
    ("3.5.9","2026-04-28","patch",  "CC 수정: verify read 결과로 data/orig_data 동기화 → dirty 표시 방지"),
    ("3.5.8","2026-04-28","patch",  "CP2112 sclLowTimeout=False — NVM 클럭 스트레칭 허용"),
...
class CP2112I2C:
    ...
    def write_burst(self, i2c_addr_8bit, start_reg, data_list):
        """연속 주소 쓰기 — 내부적으로 byte-by-byte 실행
        CMIS 모듈 대부분은 2바이트([REG][DATA]) 단위만 허용.
        3바이트 이상 WriteRequest → 주소 NACK 발생.
        sclLowTimeout=False 설정으로 NVM 클럭 스트레칭 자연 처리됨.
        """
        self._check_open()
        for i, val in enumerate(data_list):
            self.write_byte(i2c_addr_8bit, (start_reg + i) & 0xFF, val)
        return True
    ...
    def _start_write(self):
        ...
        def task():
            ...
            try:
                ...
                # → 모듈이 자체 계산한 CC 등 RO 필드도 업데이트 (dirty 표시 방지)

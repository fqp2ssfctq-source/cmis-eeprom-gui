...
class CanvasTable(tk.Frame):
    ...
    def _start_inline_edit(self,ri):
        ...
        tk.Label(hdr,text=f"Byte 편집 ({len(bytes_list)}바이트)  Esc=취소",
        ...
        for bi,bv in enumerate(bytes_list):
            ...
            col=bi%COLS
            r=bi//COLS
            tk.Label(gf,text=f"+{bi:02X}",bg=bg,fg=acc,
...
class App(tk.Tk):
    ...
    def _build_connect(self,p):
        ...
        self.i2c_addr_var=tk.StringVar(value="A0")
    ...
    # ── 포트 ─────────────────────────────────────────────
    def _scan_i2c(self):
        """I2C 주소 스캔: 연결된 상태에서 응답 주소 탐색"""
        if not self.conn or getattr(self.conn,"simulated",False):
            messagebox.showwarning("스캔 불가","CP2112가 연결된 상태에서만 스캔 가능합니다.")
            return
        ...
        def task():
            try:
                found=self.conn.scan_addresses()
                ...
                self.after(0,lambda:self.conn_status.config(
                    text="● CP2112 연결됨",fg=self.theme["grn"]))
        import threading
    ...
    def _toggle_connect(self):
        if self.conn:
            ...
            self.conn_btn.config(text="연결"); self._log("연결 해제"); return
        if not HID_OK:
            if messagebox.askyesno("DLL 없음",
                "SLABHIDtoSMBus.dll을 스크립트 폴더에 복사하세요.\n\n시뮬레이션 모드로 연결?"):
                self.conn=SimulatedI2C()
        ...
        try:
            self.conn=CP2112I2C(device_index=idx)
            self.conn_status.config(text="● CP2112 연결됨",fg=self.theme["grn"])
            self.conn_btn.config(text="연결 해제")
            self._log(f"CP2112 연결: 장치[{idx}]"); self._update_conn_info()
        except Exception as e:
    ...
    def _read_eeprom(self):
        ...
        def task():
            try:
                ...
                # Lower Memory: reg 0x00부터 128바이트 한번에 읽기
                data=self.conn.read_page(i2c, 0x00, 128)
                for reg,val in enumerate(data): self.data["a0"][reg]=val
                self._log("Lower Memory 완료")
                for pk,pn in zip(["p00","p01","p02","p10","p11"],[0,1,2,0x10,0x11]):
                    self.conn.set_page(pn)
                    # Upper Memory: reg 0x80부터 128바이트 한번에 읽기
                    data=self.conn.read_page(i2c, 0x80, 128)
                    for reg,val in enumerate(data): self.data[pk][reg]=val
                    self._log(f"Page {pk} 완료")
                self.orig_data=copy.deepcopy(self.data)
                self.after(0,lambda:self._show_page(self.current_page))
                self._log("✓ 읽기 완료")
            except Exception as e:
                logging.error(f"EEPROM 읽기 오류", exc_info=True)
                self._log(f"읽기 오류: {e} (crash.log 참조)")
                # 연결 끊김이면 GUI 상태도 미연결로 갱신
                if self.conn and not getattr(self.conn,"_is_open",True):
                    self.conn=None
                    self.after(0,lambda:(
                        self.conn_status.config(text="● 미연결 (연결 끊김)",
                                                fg=self.theme["red"]),
    ...
    def _start_write(self):
        ...
        def task():
            try:
                ...
                for pk in PAGE_KEYS:
                    if not self.write_vars[pk].get(): continue
                    if pk!="a0": self.conn.set_page(page_map[pk]); base=0x80
                    else: base=0
                    for i in range(s,min(e+1,128)):
                        if dirty_only and self.data[pk][i]==self.orig_data[pk][i]: continue
                        self.conn.write_byte(i2c,base+i,self.data[pk][i])
                ...
                logging.error(f"EEPROM 쓰기 오류", exc_info=True)
    ...
    def _verify_write(self):
        ...
        def task():
            try:
                i2c=i2c_8bit(self.i2c_addr_var.get())  # 7bit→8bit
                rb=self.conn.read_page(i2c, 0x00, 128)
            ...
            except Exception as ex: self._log(f"검증 오류: {ex}")
        threading.Thread(target=task,daemon=True).start()
